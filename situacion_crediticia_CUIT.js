...
const URL_BCRA_CUIT = 'http://www.bcra.gov.ar/BCRAyVos/Situacion_Crediticia.asp'

const processParams = {
    numeroCuit: process.argv[2]
}
...
const checkCuit = async() =>{
    return new Promise(async function(resolve, reject) {
        try{
            console.log(processParams.numeroCuit)
            let numeroCuit = processParams.numeroCuit

            if(numeroCuit.includes('-')){
                numeroCuitSeparate = numeroCuit.split('-')
                 numeroCuit = numeroCuitSeparate[0]+numeroCuitSeparate[1]+numeroCuitSeparate[2]
            }
            console.log(numeroCuit)
           // browser.close()
        await retry(async bail => {
            await processDataRequest(numeroCuit)
        })
...
const procesarReCaptcha = async () => {
    return new Promise(async function(resolve, reject) {
        ...
        anticaptcha.setWebsiteURL(URL_BCRA_CUIT)
    ...
    return new Promise(async function(resolve, reject) {
        try {
            ...
            if (verificacion.includes('No existen deudas registradas para el CUIT-CUIL-CDI')){
                console.log('No existen deudas registradas para el CUIT-CUIL-CDI '+processParams.numeroCuit)
                process.exit()
            }
...
const processDataRequest = async (numeroCuit) => {
    return new Promise(async function(resolve, reject) {
           try {
            ...
            await page.type('input.form-control',numeroCuit)
...
const run = async () => {
    ...
    try {
        ...
        const processResult = await checkCuit()
